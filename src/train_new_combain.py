...
def get_args_parser():
    ...
    parser.add_argument('--future', default=21, type=int)
...
def train(args, seed, np, resume_recent, x_train, y_train, x_test, y_test, test_name, x_test2, y_test2, test_name2, dims=None,
                  ensemble=False,
                  prints=False):
    if dims is None:
        dims = [8]
    ...
    ckpt_day = int(resume_recent.split('/')[-1].split('_')[1])
    recent_model = ensamble_encoder(x_train.shape[-1], 1, np, dims, day=ckpt_day, activation='gelu',
                                              norm=args.norm,  dropout=args.dropout,
                                              model=network.__dict__[args.used_model]).to(device)
    checkpoint_paths = [resume_recent + f'_{p}.pth' for p in range(np)]
    network.load_pretrained_models(recent_model, checkpoint_paths)
    ...
    for epoch in range(num_epochs):
        for i, (images, labels) in enumerate(train_loader):
            ...
            with torch.no_grad():
                recent = recent_model(images)

            # Forward pass
            outputs = model(images, recent)
    ...
    with torch.no_grad():
        ...
        for images, labels, names in test_loader:
            ...
            if 'reg_classify' in args.used_model:
                ...
                pred.append(outputs.detach().cpu().squeeze().reshape([images.shape[0]]))
            true.append(labels.detach().cpu().squeeze().reshape([images.shape[0]]))
    ...
    return mae, mae2, mse, mse2, cmae, cmae2, cmse, cmse2, model, recent_model
...
def test(args, model, recent_model, x_test, y_test, test_name, ensemble=False, prints=False):
    batch_size = args.batch_size
    ...
    with torch.no_grad():
        ...
        for images, labels, names in test_loader:
            ...
            if 'reg_classify' in args.used_model:
                ...
                pred.append(outputs.detach().cpu().squeeze().reshape([images.shape[0]]))
            true.append(labels.detach().cpu().squeeze())
...
def main(args):
    ...
    GBR_start_date_train, GBR_original_dict_train, GBR_dict_test, GBR_df_sort = load_data_new_time(
        f'{args.anno_path}/collect_date_{args.dataset_version}', 'GBR', suffix=args.suffix,
        consecutive_days=consecutive_days, new_split=args.new_split)
    USA_start_date_train, USA_original_dict_train, USA_dict_test, USA_df_sort = load_data_new_time(
        f'{args.anno_path}/collect_date_{args.dataset_version}', 'USA', suffix=args.suffix,
        consecutive_days=consecutive_days, new_split=args.new_split)
    ...
    GBR_train_set, GBR_train_label, _, _, _, GBR_linage, GBR_exp_set, exp_linage, GBR_exp_label, GBR_samll_label, GBR_less_label, GBR_lag_label = \
        generate_cross_validation_one_country_new(b1, b3, dict_var, sbar, max_day=max_day, days=day, exp_days=exp_days,
                                                  ...
                                                  smoothed_label=GBR_smoothed_dict_test, noise_level=args.noise_level, drop_rule=args.drop_rule,
    ...
    USA_train_set, USA_train_label, _, _, _, USA_linage, USA_exp_set, exp_linage, USA_exp_label, USA_samll_label, USA_less_label, USA_lag_label = \
        generate_cross_validation_one_country_new(b2, b4, dict_var, sbar, max_day=max_day, days=day, exp_days=exp_days,
                                                  ...
                                                  smoothed_label=USA_smoothed_dict_test, noise_level=args.noise_level, drop_rule=args.drop_rule,
                                                  seq_out=args.seq_out)
    ...
    for ckpt_name, s, um, d in zip(
            ['final_drop_token', 'final_drop_token_latest', 'final_drop_token_latest2'],
            [0]*3, ['transformer_encoder2'] * 3, [14]*3):
        checkpoint_path = f'{args.ckpt_dir}/{um + ckpt_name}/label_{args.label_type}'
        checkpoint_list.append(checkpoint_path + f'/seed{s}/checkpoint_{42}_{d}_all2_seed{s}')
    
    resume_recent = checkpoint_list[args.recent]

    seed_list = args.seed_list if len(args.seed_list) else list(range(0, 16))
    ...
    for i, seed in enumerate(seed_list):
        ...
        for p, ind in enumerate(pop_list):
            ...
            current_mae1, current_mae2, current_mse1, current_mse2, current_cmae1, current_cmae2, current_cmse1, current_cmse2, model, recent_model = train(args, seed, len(pop_list), resume_recent, x_train, y_train,
            ...
            if args.test_on_new:
                ...
                _, _, current_tmae1, current_tmse1, current_tcmae1, current_tcmse1, _, _, _ = test(args, model, recent_model, test_set, test_label, test_name,
                                                                         ensemble=False, prints=False)

                test_set, test_label = np.concatenate(USA_train_set3, axis=0), np.concatenate(USA_train_label3, axis=0)
                test_name = USA_linage3[0]
                _, _, current_tmae2, current_tmse2, current_tcmae2, current_tcmse2, _, _, _ = test(args, model, recent_model, test_set, test_label, test_name,
                                                                         ensemble=False, prints=False)
